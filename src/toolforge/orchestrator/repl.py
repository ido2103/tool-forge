...
async def _amain(args: argparse.Namespace) -> None:
    ...
    # The REPL has a human on stdin, so ask_user is wired here; headless hosts
    # pass ask_user=None and the model never sees the schema.
    host = build_host(
        anthropic,
        orch_settings,
        sandbox_settings,
        ...
        hooks=hooks,
        ask_user=_ask_via_stdin,
    )
    orch, sandbox, candidates = host.orchestrator, host.sandbox, host.candidates
    ...
    if host.loaded_tools:
        print(
            _dim(
                f"(loaded {len(host.loaded_tools)} forged tool(s): {', '.join(host.loaded_tools)})"
            )
        )
