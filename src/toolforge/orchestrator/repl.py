...
from toolforge.providers import AnthropicClient, Message, OpenAICompatClient, ProviderClient
...
def _build(
    ...
    # Sharing the REPL HookManager narrates the build live via the same
    # pre/post tool one-liners the orchestrator's calls get.
    worker = ForgeWorker(
        worker_client,
        sandbox,
        sandbox_settings,
...
async def _amain(args: argparse.Namespace) -> None:
    ...
    worker_settings = WorkerSettings()
    test_author_settings = TestAuthorSettings()
    orch, sandbox, candidates, system_prompt = _build(
        anthropic, orch_settings, sandbox_settings, worker_settings, test_author_settings
    )
